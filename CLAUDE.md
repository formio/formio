...
- **Using collation, `$lookup` pipelines, compound nested-path indexes, or other beyond-baseline Mongo features** → gate on `mongoFeatures` (probed at [`src/db/index.js:262-296`](./src/db/index.js)) and ship a fallback; consumers exist in formio-server too. Reverted-twice evidence: FIO-11254/FIO-3899 (#804). See [`formio/mongo-features-detection-01`](../../docs/gotchas/formio.md#mongo-features-detection-01--the-server-must-run-on-mongodb-documentdb-and-cosmosdb-mongofeatures-startup-probes-are-the-only-capability-gate).
