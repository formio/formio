        ...
        async.eachSeries(
          ...
          (action, cb) => {
            this.shouldExecute(action, req, res).then((execute) => {
              ...
              // Resolve the action.
              log.debug(`Action name: ${action.name}, title: ${action.title}`);
          ...
          (err) => {
            if (err) {
              log.error(err, 'Actions execution fail');
              return next(err);
            }
    ...
    async shouldExecute(action, req, res) {
      ...
      if (condition.custom) {
        ...
        try {
          ...
          log.error({ err: err.message || err }, 'Error during executing action custom logic');
          return false;
        }
      }
      ...
      if (!_.isEmpty(condition.field) && !_.isEmpty(condition.eq)) {
        ...
        // `value` is submission data; log only which condition is being evaluated.
        log.debug({ field, eq }, 'Evaluating action condition');
    ...
    async.eachSeries(
      ...
      (action, callback) => {
        action.info(req, res, (err, info) => {
          if (err) {
            log.error(err, "Error, can't get action info");
            return callback(err);
          }
      ...
      (err) => {
        if (err) {
          log.error(err, 'Error during actions info parsing');
          return next(err);
        }
    ...
    action.info(req, res, async (err, info) => {
      ...
      try {
        ...
        try {
          ...
          log.error(err, "Error, can't get action settings");
          return res.status(400).send(err);
        }

        action.settingsForm(req, res, (err, settingsForm) => {
          if (err) {
            log.error(err, "Error, can't get form settings");
            return next(err);
          }
