...
class CSVExporter extends Exporter {
  constructor(form, req, res) {
    ...
    try {
      util.eachComponent(
        form.components,
        (comp, path, components, parent, compPaths) => {
          ...
          if (component.type === 'address') {
            ...
            items.push({
              ...
              preprocessor: (value) => {
                ...
                return (
                  ...
                  _.get(address, 'address.freeformAddress') ||
            ...
            items.push({
              ...
              preprocessor: (value) => {
                ...
                return (
                  address.lat ||
                  _.get(address, 'position.lat') ||
                  _.get(address, 'geometry.location.lat') ||
                  ''
                );
              },
            });
            items.push({
              ...
              preprocessor: (value) => {
                ...
                return (
                  address.lon ||
                  _.get(address, 'position.lon') ||
                  _.get(address, 'geometry.location.lng') ||
                  ''
                );
              },
            });
            ...
            items.push({
              preprocessor: (value) => {
                ...
                const componentValue =
                  component.values.find((v) => v.value === value.toString()) || '';
  ...
  stream(stream) {
    ...
    return stream.pipe(through(write, () => this.res.end())).pipe(this.stringifier);
  }
  ...
  coerceToString(data, column, submission) {
    ...
    if (Array.isArray(data) && data.length > 0) {
      const fullPath = column.subpath ? `${column.key}.${column.subpath}` : column.key;

      return data
        .map((item) => `${this.coerceToString(_.get(item, fullPath, item), column, submission)}`)
        .join(',');
