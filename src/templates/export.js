...
module.exports = (router) => {
  ...
  const exportActions = async function (_export, _map, options, next) {
    try {
      const actions = await formio.actions.model
        .find({
      ...
      _.each(actions, function (action) {
        ...
        const machineName = (action.machineName = hook.alter(
          'machineNameExport',
          action.machineName,
        ));
        _export.actions[machineName] = _.pick(
          action,
          'title',
          'name',
          'form',
  ...
  const exportForms = async function (_export, _map, options, next) {
    ...
    try {
      ...
      _.each(forms, function (form) {
        ...
        _export[`${form.type}s`][machineName] = _.pick(
          form,
          'title',
          'type',
          'name',
    ...
    try {
      const reports = await formio.resources.submission.model
        .find(
          hook.alter(
            'submissionQuery',
            { form: reportingConfigurationFormId, deleted: { $eq: null } },
            options,
          ),
        )
        .lean(true)
  ...
  const exportRevisions = async function (_export, _map, options, next) {
    if (_map.revisions.revisionsData.length > 0) {
      ...
      _.each(_map.revisions.revisionsData, (revision) => {
        if (revision.revision.length === 24) {
          ...
          if (_map.revisions.formsWithEnabledRevisions.length > 0) {
            const form = _map.revisions.formsWithEnabledRevisions.find(
              (form) => form.machineName === revision.form,
            );
            if (form) {
              revisionsArray.push({
                project: form.project,
      ...
      if (revisionsArray.length > 0) {
        ...
        try {
          const revisions = await hook.alter('formRevisionModel').find(query).lean(true);
          if (
            revisions &&
            revisions.length > 0 &&
            ...
            revisions.forEach((revision) => {
              const component = _map.revisions.revisionsData.find(
                (component) => component.form === revision.name,
              );
              if (component) {
                ...
                _export.revisions[`${machineName}:${componentRevision}`] = _.pick(
                  revision,
                  'title',
                  'type',
                  'name',
                ...
                const form = _map.revisions.formsWithEnabledRevisions.find(
                  (form) => form.machineName === revision.name,
                );
                if (form) {
  ...
  const exportRoles = async function (_export, _map, options, next) {
    try {
      ...
      _.each(roles, function (role) {
        ...
        _export.roles[machineName] = _.pick(role, 'title', 'description', 'admin', 'default');
    ...
    router.get('/export', async (req, res, next) => {
      ...
      if (options) {
        options.includeFormFields =
          req.query.include && req.query.include.split(',').filter((field) => !!field);
      }
