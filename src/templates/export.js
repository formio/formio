...
module.exports = (router) => {
  ...
  // Export actions.
  const exportActions = async function(_export, _map, options) {
  ...
  // Export forms.
  const exportForms = async function(_export, _map, options) {
  ...
  // Export reports.
  const exportReports = async function(_export, _map, options) {
  ...
  const exportRevisions = async function(_export, _map, options) {
  ...
  // Export the roles.
  const exportRoles = async function(_export, _map, options) {
   ...
   */
  const exportTemplate = async (options) => {
    const basicTemplate = {
      title: 'Export',
      version: '2.0.0',
      description: '',
    ...
    const exportSteps = hook.alter(`templateExportSteps`,[
      ()=>exportRoles(template, map, options),
      ()=>exportForms(template, map, options),
      ()=>exportActions(template, map, options),
      ()=>exportRevisions(template, map, options),
      ()=>exportReports(template, map, options),
