  ...
  _.each([
    ...
    var initializeSubmission = function(req, done) {
      ...
      if (!isGet && req.body) {
        ...
        var properties = hook.alter('submissionParams', ['data', 'owner', 'access', 'metadata']);
    ...
    var validateSubmission = function(req, res, done) {
      ...
      const validator = new Validator(req.currentForm, router.formio.resources.submission.model, token);
