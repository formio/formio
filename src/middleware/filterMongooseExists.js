...
module.exports = (router) => (settings) => function(req, res, next) {
  ...
  const submissionModel = this.model.modelName === 'submission' ? req.submissionModel : null;
  req.modelQuery = req.modelQuery || req.model || submissionModel || this.model;
  req.modelQuery = req.modelQuery.find(query);

  req.countQuery = req.countQuery || req.model || submissionModel || this.model;
