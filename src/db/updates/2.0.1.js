      ...
      .forEach(
        function (project) {
          ...
          let getRoles = function (next) {
            roles
              .find({ query: { project: project._id } })
              .toArray()
              .then((docs) => {
          ...
          let saveProject = function (next) {
            projects
              .updateOne({ _id: project._id }, { $set: { access: project.access } })
    ...
    forms
      ...
      .forEach(
        function (form) {
          ...
          let getRoles = function (next) {
            roles
              .find({ query: { project: form.project } })
              .toArray()
              .then((docs) => {
          ...
          let saveForm = function (next) {
            forms
              .updateOne(
                { _id: form._id },
                { $set: { access: form.access, submissionAccess: form.submissionAccess } },
              )
              .then((result) => next(null, result))
              .catch((err) => next(err));
          };
