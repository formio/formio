      ...
      .then((docs) => {
        async.forEachOf(
          ...
          function (project, key, next) {
            ...
            projects
              .updateOne({ _id: project._id }, { $set: { machineName: machineName } })
              .then(() => next())
              .catch((err) => next(err));
          },
          function (err) {
          ...
          function (form, key, next) {
            projects
              ...
              .then((project) => {
                ...
                forms
                  .updateOne({ _id: form._id }, { $set: { machineName: machineName } })
                  .then(() => next())
                  .catch((err) => next(err));
              })
              .catch((err) => next(err));
  ...
  let updateActions = function (callback) {
    ...
    actions
      ...
      .then((docs) => {
        async.forEachOf(
          docs,
          function (action, key, next) {
            forms
              .findOne({ _id: action.form })
              .then((form) => {
                let machineName = '';
                if (form) {
                ...
                actions
                  .updateOne({ _id: action._id }, { $set: { machineName: machineName } })
                  .then(() => next())
                  .catch((err) => next(err));
              })
              .catch((err) => next(err));
  ...
  let updateRoles = function (callback) {
    ...
    roles
      ...
      .then((docs) => {
        async.forEachOf(
          docs,
          function (role, key, next) {
            projects
              .findOne({ _id: role.project })
              .then((project) => {
                let machineName = '';
                if (project) {
                  machineName = project.machineName + ':';
                ...
                roles
                  .updateOne({ _id: role._id }, { $set: { machineName: machineName } })
                  .then(() => next())
                  .catch((err) => next(err));
              })
              .catch((err) => next(err));
