...
module.exports = function (db, config, tools, done) {
  ...
  let makeUnique = function (form, cb) {
    ...
    while (
      form.path.match(invalid) ||
      projPaths[form.project.toString()].indexOf(comparison) !== -1
    ) {
      ...
      .then((docs) => {
        async.eachSeries(
          ...
          function (form, cb) {
            ...
            if (
              form.path.match(invalid) ||
              projPaths[form.project.toString()].indexOf(form.path) !== -1
            ) {
              makeUnique(form, cb);
            } else {
  ...
  forms
    ...
    .then((docs) => {
      async.eachSeries(
        ...
        function (form, cb) {
          forms
            .updateOne({ _id: form._id }, { $set: { path: form.name.toLowerCase() } })
            .then(() => cb())
            .catch((err) => cb(err));
        },
        function (err) {
