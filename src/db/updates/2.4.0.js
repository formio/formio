      ...
      function (form) {
        ...
        utils.eachComponent(form.components, function (component) {
          ...
          componentPromises.push(
            forms.findOne({ name: keyParts[0], project: form.project }).then(function (resource) {
              if (resource === null) {
                // Embedded component had bad name, attempt to fix by camel casing name
                return forms
                  .findOne({ name: _.camelCase(keyParts[0]), project: form.project })
      ...
      function () {
        Q.all(formPromises)
          .then(function (changedForms) {
            ...
            return Q.all(
              changedForms.map(function (form) {
