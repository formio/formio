...
module.exports = function(app, template, hook) {
  ...
  describe('Form Submissions', function() {
    ...
    it('', function(done) {
      const formSchema = require('./fixtures/forms/formWithRevisions.js');
      const testSubmission = {
        data: {
          textField: 'test',
          container: {
      ...
      helper.upsertForm(formSchema, (err, form) => {
        if (err) {
          return done(err);
        }

        helper.updateForm({...form, revisions: 'current'}, (err) => {
          if (err) {
            return done(err);
          }
          ...
          helper
            ...
            .execute((err) => {
              ...
              let updatedSubmission = _.cloneDeep(helper.getLastSubmission());
              ...
              helper.updateForm(updatedForm, (err, form) => {
                if (err) {
                  return done(err);
                }
                ...
                helper
                  ...
                  .execute((err) => {
                    ...
                    assert.deepEqual(
                      ...
                      data,
                      'Should keep the fields that do no exist on the current form revisions and do not change it',
                    );
