...
module.exports = function (app, template, hook) {
  ...
  describe('Resources', function () {
    ...
    // Every test below reads `template.resources.tempResource`, which the first test creates.
    // The create lives in a guarded builder so that each of those tests can also run on its own;
    // in whole-file order the first test still builds it and every later call is a no-op.
    var ensureTempResource = function (done) {
      if (template.resources.tempResource) {
        return done(null, template.resources.tempResource);
      }
      ...
      request(app)
        ...
        .end(function (err, res) {
          ...
          template.resources.tempResource = res.body;
    ...
    describe('Permissions - Resource Level - Project Owner', function () {
      it('An administrator should be able to Create a Resource', function (done) {
        ensureTempResource(function (err, response) {
          if (err) {
            return done(err);
          }
      ...
      it('A Project Owner should be able to Read a Resource', function (done) {
        ensureTempResource(function (err) {
          if (err) {
            return done(err);
          }
          ...
          request(app)
            ...
            .end(function (err, res) {
              ...
              assert.deepEqual(
                ...
                _.omit(template.resources.tempResource, ignoreFields),
              );
        ...
        ensureTempResource(function (err) {
          ...
          request(app)
            ...
            .end(function (err, res) {
              ...
              assert.deepEqual(
                _.omit(response, ignoreFields),
                _.omit(updatedResource, ignoreFields),
              );
