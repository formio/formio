...
module.exports = function (app, template, hook) {
  ...
  describe('Validator tests', function () {
    ...
    itCoreOnly(
      'Should run the encapsulated single-sweep path when opted in, matching default results',
      async function () {
        const form = {
          components: [
    ...
    itCoreOnly(
      'encapsulated mode does not re-evaluate custom conditionals per-expression during validation',
      async function () {
    ...
    itCoreOnly(
      'encapsulated mode yields the same validation result and data as the default path',
      async function () {
        const form = {
          components: [
            { type: 'number', key: 'a', input: true },
    ...
    itCoreOnly(
      'encapsulated mode does not validate phantom rows of an absent data grid in a nested form (matches default)',
      async function () {
        const form = {
          components: [
            { type: 'textfield', key: 'req', input: true, validate: { required: true } },
    ...
    itCoreOnly(
      'encapsulated mode exposes the form/project public config and headers to custom logic (matches default)',
      async function () {
        const form = {
    ...
    itCoreOnly(
      'encapsulated mode fails the request when custom logic exceeds the VM timeout',
      async function () {
        this.timeout(5000);
        const form = {
          components: [
    ...
    it('Instance should be available in evaluation context for content components', async function () {
      ...
      const validator = new Validator(req, { formio });
      const submission = {
        data: { textField: 'test' },
      };
      await validator.validate(submission, (err, data) => {
        if (IS_NEXTGEN) {
          assert.deepEqual(req.nextgenHidden, ['html']);
        } else {
