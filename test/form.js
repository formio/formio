  ...
  describe('Forms', function () {
      ...
      it('A Project Owner should be able to Read their Index of Resources', function (done) {
        request(app)
          ...
          .end(function (err, res) {
            ...
            assert.equal(
              _.difference(_.map(res.body, ids), _.map(template.resources, ids)).length,
              0,
            );
      ...
      it('An administrator should be able to Create a Register Form', function (done) {
        ...
        request(app)
          ...
          .end(function (err, res) {
            ...
            assert.equal(
              response.submissionAccess[0].roles[0],
              template.roles.anonymous._id.toString(),
            );
            assert.equal(response.access.length, 1);
            assert.equal(response.access[0].type, 'read_all');
      ...
      it('Should be able to create a form with the type=form', function (done) {
        request(app)
          ...
          .end(function (err, res) {
            ...
            new Helper(template.users.admin, template, hook).deleteForm(res.body).execute(done);
          });
      });
      ...
      it("An Anonymous user should not be able to Read their Form's components", function (done) {
        request(app)
          .get(
            hook.alter(
              'url',
              '/form/' + template.forms.testComponentForm._id + '/components',
              template,
            ),
          )
          .expect(401)
      ...
      it("An Administrator should be able to Read and filter their Form's components with a string", function (done) {
        request(app)
          .get(
            hook.alter(
              'url',
              '/form/' + template.forms.testComponentForm._id + '/components?type=email',
              template,
            ),
          )
          .set('x-jwt-token', template.users.admin.token)
          ...
          .end(function (err, res) {
            ...
            assert.deepEqual(
              response[0],
              _.find(template.forms.testComponentForm.components, { type: 'email' }),
            );
      ...
      it("An Administrator should be able to Read and filter their Form's components by existence", function (done) {
        request(app)
          .get(
            hook.alter(
              'url',
              '/form/' + template.forms.testComponentForm._id + '/components?validate.step',
              template,
            ),
          )
          .set('x-jwt-token', template.users.admin.token)
          ...
          .end(function (err, res) {
            ...
            assert.deepEqual(
              response[0],
              _.find(template.forms.testComponentForm.components, { validate: { step: 23 } }),
            );
      ...
      it("An Administrator should be able to Read and filter their Form's components with a number", function (done) {
        request(app)
          .get(
            hook.alter(
              'url',
              '/form/' + template.forms.testComponentForm._id + '/components?validate.step=23',
              template,
            ),
          )
          .set('x-jwt-token', template.users.admin.token)
      ...
      it("An Administrator should be able to Read and filter their Form's components with the boolean `true`", function (done) {
        request(app)
          .get(
            hook.alter(
              'url',
              '/form/' + template.forms.testComponentForm._id + '/components?protected=true',
              template,
            ),
          )
          .set('x-jwt-token', template.users.admin.token)
          ...
          .end(function (err, res) {
            ...
            assert.deepEqual(
              response[0],
              _.find(template.forms.testComponentForm.components, { protected: true }),
            );
      ...
      it("An Administrator should be able to Read and filter their Form's components with the boolean `false`", function (done) {
        request(app)
          .get(
            hook.alter(
              'url',
              '/form/' + template.forms.testComponentForm._id + '/components?protected=false',
              template,
            ),
          )
          .set('x-jwt-token', template.users.admin.token)
          ...
          .end(function (err, res) {
            ...
            assert.deepEqual(
              response[0],
              _.find(template.forms.testComponentForm.components, { protected: false }),
            );
            assert.deepEqual(
              response[1],
              _.findLast(template.forms.testComponentForm.components, { protected: false }),
            );
        ...
        request(app)
          .get(
            hook.alter(
              'url',
              '/form/' +
                template.forms.testComponentForm._id +
                '/components?protected=false&validate.step=23&type=number',
              template,
            ),
          )
          .set('x-jwt-token', template.users.admin.token)
          ...
          .end(function (err, res) {
            ...
            assert.deepEqual(
              response[0],
              _.find(template.forms.testComponentForm.components, {
                protected: false,
                type: 'number',
                validate: { step: 23 },
              }),
      ...
      it('An Administrator should receive empty array when Reading their Form with filter that has no results', function (done) {
        request(app)
          .get(
            hook.alter(
              'url',
              '/form/' + template.forms.testComponentForm._id + '/components?type=2342342344234',
              template,
            ),
          )
          .set('x-jwt-token', template.users.admin.token)
    ...
    describe('Form Validation', function () {
          ...
          request(app)
            ...
            .end(function (err, res) {
              ...
              assert.equal(
                ...
                'foo must have no more than -1 characters.',
              );
              done();
            });
        });
      ...
      describe('Duplicate form component keys', function () {
        ...
        it('A form can be created with unique component api keys nested within layout components (FIO-10591)', async function () {
          const form = {
            name: 'fio10591testform',
            title: 'FIO 10591 Test Form',
            path: 'fio10591testform',
            components: [
              {
                type: 'textfield',
                validate: {
              ...
              // Nest an identically-keyed component in a layout component like columns
              {
                label: 'Columns',
                columns: [
                  {
              ...
              function (path, callback) {
                ...
                request(app)
                  ...
                  .end(function (err, res) {
                    ...
                    assert.equal(
                      response,
                      'Form path cannot contain one of the following names: ' +
                        formio.config.reservedForms.join(', '),
                    );
      ...
      describe('Unique fields are case insensitive', function () {
        ...
        it('Unique field data is stored in its original submission state', function (done) {
          request(app)
            .get(
              hook.alter('url', '/form/' + testEmailForm._id + '/submission', template) +
                '/' +
                sub._id,
            )
            .set('x-jwt-token', template.users.admin.token)
        ...
        it('Should return custom message for unique validation error', async function () {
          ...
          const result = await request(app)
            .post(
              hook.alter(
                'url',
                '/form/' + template.forms.uniqueValidationCustomMessage._id + '/submission',
                template,
              ),
            )
            .set('x-jwt-token', template.users.admin.token)
        ...
        after((done) => {
          request(app)
            .delete(
              hook.alter(
                'url',
                '/form/' + template.forms.uniqueValidationCustomMessage._id,
                template,
              ),
            )
            .set('x-jwt-token', template.users.admin.token)
      ...
      describe('Unique fields work inside layout components', function () {
        ...
        it('Unique field data is stored in its original submission state', function (done) {
          request(app)
            .get(
              hook.alter('url', '/form/' + testUniqueField._id + '/submission', template) +
                '/' +
                sub._id,
            )
            .set('x-jwt-token', template.users.admin.token)
    ...
    describe('Form MachineNames', function () {
      ...
      it('Forms expose their machineNames through the api', function (done) {
        ...
        helper.form({ name: name }).execute(function (err, results) {
          if (err) {
            return done(err);
          }
      ...
      it('A user can modify their form machineNames', function (done) {
        ...
        helper.form({ name: name }).execute(function (err, results) {
          ...
          helper.form(form).execute(function (err, results) {
            if (err) {
              return done(err);
            }
    ...
    describe('Reference Components', function () {
      ...
      it('Should be able to filter and sort and pick certain fields of the list of references', (done) => {
        let url = '/form/' + referenceForm._id + '/submission';
        url +=
          '?data.user.data.firstName=Joe&sort=data.user.data.lastName&select=data.user.data.firstName,data.user.data.lastName';
        request(app)
          .get(hook.alter('url', url, template))
          .set('x-jwt-token', template.users.admin.token)
          .expect('Content-Type', /json/)
      ...
      it('Should be able to load the full data when a GET routine is retrieved.', (done) => {
        request(app)
          .get(
            hook.alter(
              'url',
              '/form/' + referenceForm._id + '/submission/' + references[0]._id,
              template,
            ),
          )
          .set('x-jwt-token', template.users.admin.token)
