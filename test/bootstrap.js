  ...
  describe('Bootstrap test data', function () {
    ...
    it('Should be able to bootstrap the default template', function (done) {
      ...
      app.formio.template.import.template(template, function (err) {
        ...
        assert.equal(
          resourceA._id,
          resourceComponentA.resource,
          `Resource B's resource component for A should have the correct resource id. (Got ${resourceComponentA.resource}, expected ${resourceA._id})`,
        );
        assert.equal(
          resourceB._id,
          resourceComponentB.resource,
          `Resource A's resource component for B should have the correct resource id. (Got ${resourceComponentB.resource}, expected ${resourceB._id})`,
        );
        done();
      });
    });
