...
export const NewForm = ({ type }: { type: 'form' | 'resource' }) => {
  ...
  return (
    <div className={`panel-wrap content fio-card remember-focus-content new-item active ${type}`}>
      <div className="panel-header">
        <div className="panel-header-section top">
          <div className="panel-title icon">
            <img src={`icon-${type}.svg`} alt={`${type} Icon`} /> New {name}
          </div>
          <button
      ...
      <div className="panel new-item">
        <FormEdit
          ...
          components={{
            ...
            BuilderContainer: ({ children }) => <div style={{ padding: '15px' }}>{children}</div>,
