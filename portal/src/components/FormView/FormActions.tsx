...
const isButtonComponent = (comp: Component): comp is ButtonComponent =>
    comp.type === "button";
...
export const FormActions = ({
    ...
    const [displayedAction, setDisplayedAction] = useState<FormAction | null>(
        null
    );
    ...
    const handleEditAction = async (id: string, name: string) => {
        try {
            ...
            const [action, actionInfo] = await Promise.all([
                actionPromise,
                actionInfoPromise,
            ]);
            ...
            const submitButton = settingsForm.components.find(
                (comp: Component) =>
                    isButtonComponent(comp) && comp.action === "submit"
            );
            if (!submitButton) {
                ...
                {actionsToMap.map((action) => (
                    <div className="form-action-row action" key={action._id}>
                        <div className="form-action-name">
                            <button
                                className="textlink"
                                onClick={() => handleActionClick(action._id)}
                            >
                ...
                <div className="form-action-row add">
                    <div className="form-action-add-wrap">
                        ...
                        <button
                            ...
                            onClick={() =>
                                actionToAdd.current &&
                                handleAddAction(actionToAdd.current)
                            }
                        >
