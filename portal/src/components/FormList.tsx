...
const FormActionsContainer: FormGridComponentProps['FormActionsContainer'] = ({ children }) => (
  <div className="item-buttons">{children}</div>
);

const FormActionButton: FormGridComponentProps['FormActionButton'] = ({ action, onClick }) => (
  <a className={`btn ${action && action.name === 'Edit' ? 'edit' : 'trash'}`} onClick={onClick}>
    <i
...
const PaginationContainer: FormGridComponentProps['PaginationContainer'] = ({ children }) => (
    ...
    <div className="panel-header">
      <div className="panel-header-section top">
        <div className="panel-title icon">
          <img src={`icon-${type}.svg`} alt={`${type}s`} /> {pluralName}
          <button
        ...
        <div className={`help-bubble ${isBubbleActive ? 'active' : ''} top wide`}>
          <button className="close-help" type="button" onClick={() => setBubbleActive(false)}>
      ...
      <div className="panel-header-section bottom">{/* <SearchForm /> */}</div>
    </div>
  );
};
...
export const FormList = ({ type }: { type: 'form' | 'resource' }) => {
  ...
  const FormContainer: FormGridComponentProps['FormContainer'] = ({ children }) => (
